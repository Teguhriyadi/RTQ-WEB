...
@section('app_title', 'Besaran Santri')
    ...
    <div class="row">
        <div class="col-md-4 col-sm-5 col-xs-12">
            <div class="x_panel">
                ...
                <div class="x_content">
                    <form method="POST" action="{{ url('/app/sistem/besaran_santri') }}">
                        @csrf
                                ...
                                <table id="datatable" class="table table-striped table-bordered" style="width: 100%">
                                    ...
                                    <tbody>
                                        @php
                                            $no = 0;
                                        @endphp
                                        @foreach ($data_besaran_santri as $data)
                                            <tr>
                                                <td class="text-center">{{ ++$no }}.</td>
                                                <td class="text-center">Rp.
                                                    {{ number_format($data->getBesaran->besaran) }}</td>
                                                <td>{{ $data->getSantri->nama_lengkap }}</td>
                                                <td class="text-center">
                                                    <button onclick="editBesaranSantri({{ $data->id }})"
                                                        class="btn btn-warning text-white btn-sm" data-target="#modalEdit"
                                                        data-toggle="modal">
                                                        <i class="fa fa-edit"></i> Edit
                                                    </button>
                                                    <form action="{{ url('/app/sistem/besaran_santri/' . $data->id) }}"
    ...
    <div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-hidden="true" id="modalEdit">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ url('/app/sistem/besaran_santri/simpan') }}" method="POST">
                    @method('PUT')
                    {{ csrf_field() }}
...
@section('app_scripts')
    ...
    <script>
        function editBesaranSantri(id) {
            $.ajax({
                url: "{{ url('/app/sistem/besaran_santri/edit') }}",
                type: "GET",
                data: {
                    id: id
                },
