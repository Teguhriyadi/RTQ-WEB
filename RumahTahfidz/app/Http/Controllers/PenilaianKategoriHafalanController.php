...
class PenilaianKategoriHafalanController extends Controller
{
    ...
    public function create($halaqah, $id_jenjang, $id_pelajaran)
    {
        ...
        return view("app.asatidz.penilaian_per_kategori.hafalan.v_tambah", $data);
    }

    public function store($halaqah, $id_jenjang, Request $request)
    {
        foreach ($request->nilai as $index => $data) {
            $cek = NilaiHafalan::where('id_santri', $request->id_santri[$index])
                ->where('id_pelajaran_hafalan', $request->id_pelajaran)
                ->first();
            ...
            if ($cek) {
                ...
                NilaiHafalan::create([
                    ...
                    "id_pelajaran_hafalan" => $request->id_pelajaran,
                    "nilai" => $request->nilai[$index]
                ]);
            }
        }
