...
use App\Models\NilaiImla;
use App\Models\Santri;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class PenilaianKategoriImlaController extends Controller
{
    ...
    public function create($halaqah, $id_jenjang, $id_pelajaran)
    {
        ...
        return view("app.asatidz.penilaian_per_kategori.imla.v_tambah", $data);
    }

    public function store($halaqah, $id_jenjang, Request $request)
    {
        foreach ($request->nilai as $index => $data) {
            $cek = NilaiImla::where('id_santri', $request->id_santri[$index])
                ->where('id_pelajaran_imla', $request->id_pelajaran)
                ->first();

            if ($cek) {
                NilaiImla::where('id_santri', $request->id_santri[$index])->update([
                ...
                NilaiImla::create([
                    ...
                    "id_pelajaran_imla" => $request->id_pelajaran,
                    "nilai" => $request->nilai[$index]
                ]);
            }
        }
